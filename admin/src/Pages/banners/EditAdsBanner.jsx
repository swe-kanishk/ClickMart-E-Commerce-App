...
function EditAdsBanner() {
  ...
  useEffect(() => {
    getData(`/api/adsBanner/${context?.isOpenFullScreenPannel?.id}`).then(
      (res) => {
        if (res?.success === true) {
          setFormFields({
          ...
          setSubCat(res?.banner?.subCatId);
          setThirdLevelSubcat(res?.banner?.thirdLevelSubCatId);
        }
      }
    );
  ...
  const handleRemoveImage = (img, index) => {
    deleteImages("/api/adsBanner/deleteImage", img, {
  ...
  const handleSubmit = (e) => {
    ...
    setIsLoading(true);
    editData(
      `/api/adsBanner/${context?.isOpenFullScreenPannel.id}`,
      formFields,
      { withCredentials: true }
    ).then((res) => {
      console.log(res);
        ...
        <div className="scroll max-h-[78vh] pt-4 overflow-y-scroll">
          ...
          <div className="grid grid-cols-7 gap-4">
            ...
            <UploadProductBox
              ...
              multiple={false}
              name={"images"}
              url={"/api/adsBanner/uploadImages"}
            />
          </div>
        </div>
        <br />
