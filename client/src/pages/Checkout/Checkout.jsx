...
function Checkout() {
  ...
  const checkout = (e) => {
    ...
    const options = {
      ...
      currency: "INR",
      order_receipt: `order_rcptid_ ${context?.userData?.fullName}`,
      name: "ClickMart-Official",
        ...
        <div className="container flex gap-5">
              ...
              <div className="flex flex-col mt-4 gap-2">
                {context?.userData?.address_details?.length > 0 &&
                  context?.userData?.address_details?.map((address, index) => (
                    <div
                      key={index}
                      className={`addressBox group border ${
                        selectedValue === address?._id
                          ? "bg-green-50"
                          : "bg-white"
                      } border-dashed w-full flex flex-col justify-start border-gray-500 p-3 rounded-md cursor-pointer`}
                    >
                      <span
                        className={`ml-2 ${
                          selectedValue === address?._id
                            ? "bg-white"
                            : "bg-gray-200"
                        } text-[14px] font-medium w-fit py-1 rounded-md px-2`}
                      >
                        {address?.addressType}
                      </span>
                      ...
                      <div className="flex items-center w-full">
                        ...
                        <span className="text-[14px] font-[500]">
                          {address?.landmark}, {address?.address_line1},{" "}
          ...
          <div className="right-col w-[40%]">
            <div className="card bg-white shadow-md p-5 rounded-md w-full">
              ...
              <Button
                type="submit"
                className="!bg-red-500 !text-white !w-full gap-2 !flex !items-center hover:!bg-black !capitalize"
              >
